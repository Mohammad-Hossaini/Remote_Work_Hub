...
function Navbar() {
    ...
    const [avatar, setAvatar] = useState("/profile/default.jpg");

    // Update avatar from localStorage whenever it changes
    useEffect(() => {
        const authUser = JSON.parse(localStorage.getItem("authUser"));
        if (authUser?.profilePhoto) {
            setAvatar(authUser.profilePhoto);
        } else {
            setAvatar("/profile/default.jpg");
        }

        // Optional: listen to storage events if multiple tabs
        const handleStorageChange = () => {
            const updatedUser = JSON.parse(localStorage.getItem("authUser"));
            setAvatar(updatedUser?.profilePhoto || "/profile/default.jpg");
        };
